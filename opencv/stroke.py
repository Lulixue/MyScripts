...
i = 0
for cnt in drawContours:
    hole = np.zeros(imgray.shape, np.uint8)

    cv2.drawContours(hole, [cnt], -1, (255, 255,255), cv2.FILLED)

    cv2.imshow("hole" + str(i),hole)
 
    masked_img = cv2.bitwise_and(img, img, mask = hole) 
    title = "mask" + str(i)
    i+=1  
